...
def get_financial_report(ticker):
    '''
    The function takes in the stock ticker and it builds two URLs to scrape:
...
def get_element(list,element):
    '''
    Retrieve an element or returns an “ — “ if it cannot find 
    an item in the list of scraped data.
    '''
    try:
        return list[element]
    except:
