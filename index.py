...
def split_filter_part(filter_part):
    '''
    This function will be used to sort and filter through 
    refreshed Reddit and Twitter information
    '''
...
############### This callback passes the input Ticker to yfinance and returns bootstrap component Cards populated with company and historic price data. 

@app.callback(Output('cards', 'children'),
[Input('ticker-input', 'value')])
def refresh_cards(ticker):
    '''
    reload ticker information
    '''
    ticker = ticker.upper()
    if ticker is None:
            TICKER = 'MSFT'
...
def toggle_accordion(n1, n2, n3, is_open1, is_open2, is_open3):
    '''
    Sort through price chart accordian
    '''
...
def create_graph(ticker,startdate, enddate, n):
    '''
    This function creates and fills the graph for the financial reports
    '''
    ticker = ticker.upper()
...
################### Sorting and filtering the Twitter Data
...
def update_table2(page_current, page_size, sort_by, filter, n):
        '''
        This function automatically updates the Twitter section of the dashboard
        '''
        filtering_expressions = filter.split(' && ')
        conn = sqlite3.connect('stocks.sqlite')
